class BooksController < ApplicationController
  ...
  def index
    ...
    @books = @books.select('distinct library_books.*, library_reservations.book_id as bid, library_copies.state as state').joins('INNER JOIN library_copies on library_copies.book_id = library_books.id LEFT JOIN library_reservations ON library_reservations.book_id = library_books.id').order('bid DESC, state ASC') if signed_in? && current_user.admin?
